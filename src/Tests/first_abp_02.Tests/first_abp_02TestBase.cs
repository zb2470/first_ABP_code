...
{
    public abstract class first_abp_02TestBase : AbpIntegratedTestBase<first_abp_02TestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected first_abp_02TestBase()
        {
        ...
        protected void UsingDbContext(Action<first_abp_02DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<first_abp_02DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<first_abp_02DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<first_abp_02DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<first_abp_02DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<first_abp_02DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<first_abp_02DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<first_abp_02DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<first_abp_02DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<first_abp_02DbContext, Task<T>> func)
        {
            T result;
